...
function parseArgs(argv) {
  const options = { file: null, ttl: process.env.HTML_PLAN_HOST_TTL_SECONDS || null, title: null, baseUrl: process.env.HTML_PLAN_HOST_BASE_URL || DEFAULT_BASE_URL };
...
async function main() {
  ...
  const response = await fetch(`${baseUrl}/v1/plans`, {
    ...
    body: JSON.stringify({ html, title: options.title || basename(filePath), ...(ttl ? { ttl_seconds: ttl } : {}) })
  });
